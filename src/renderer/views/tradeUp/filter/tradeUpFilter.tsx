...
export default function TradeUpFilters() {
  ...
  async function updateMin(valueToSet: number) {
    if (valueToSet < tradeUpData.MaxFloat) {
      dispatch(tradeUpSetMin(valueToSet))
    }
  }
  async function updateMax(valueToSet: number) {
    if (Number(valueToSet) > tradeUpData.MinFloat) {
          ...
          <div className="max-w-7xl flex h-8 items-center space-x-6 divide-x divide-gray-200 text-sm px-4 sm:px-6 lg:px-8">
            ...
            <div className="relative rounded-md focus:outline-none">
              ...
              <input
                ...
                spellCheck="false"
                onChange={(e) => dispatch(tradeUpSetSearch({searchField: e.target.value}))}
              />
            </div>
          </div>
          <div className="flex ">
              ...
              <div className="hidden xl:block max-w-7xl flex h-8 items-center space-x-6 divide-x divide-gray-200 text-sm px-4 sm:px-6 lg:px-8">
                <div className="">
                  <div className="flex items-center justify-end">
                    <p className="text-gray-500 dark:text-gray-400 pr-3 truncate">
                      {tradeUpData.MinFloat} min 
                    </p>
                    <input type="range" min="0" max="1" value={tradeUpData.MinFloat} step="0.01" onChange={(e) => updateMin(parseFloat(e.target.value))} className=" dark:bg-dark-level-three h-2 dark:appearance-none" />
                  </div>
                  <div className="flex items-center justify-end">
                    <p className="text-gray-500 dark:text-gray-400 pr-3 truncate">
                      {tradeUpData.MaxFloat} max 
                    </p>
                    <input className=" dark:bg-dark-level-three h-2 dark:appearance-none"
                      type="range" 
                      min="0" max="1" 
                      value={tradeUpData.MaxFloat} 
                      step="0.01" 
                      onChange={(e) => updateMax(parseFloat(e.target.value))} 
                    />
                  </div>
                </div>
              </div>
